...
| `libid-ceremony` | yes | The attested-data record a notary signs: the types a Platform Profile pins, their big-endian fixed-width encoder, and the keccak256 over it that is the only preimage a notary signs. |
...
```toml
[dependencies]
libid-tlsn = { git = "https://github.com/libid-org/libID-rs", tag = "v0.6.0" }
```
